	...
	public SupplyService(
		...
		public async Task<APIOperationResponse<bool>> DeleteSupplyDetailAsync(long supplyId, long detailId)
		{
			...
			try
			{
				...
				var detailToDelete = supply.SupplyDetails?.FirstOrDefault(sd => sd.Id == detailId && !sd.IsDeleted);
				...
				// Recalculate fulfillment on the already-loaded supply (same pattern as UpdateSupplyDetailAsync)
				supply.FulfillmentStatus = CalculateFulfillmentStatus(supply);
				supply.ModificationDate = _dateTimeProvider.Now;
				supply.ModifiedBy = _currentUserService.UserId;
				await _supplyRepository.UpdateAsync(supply);
		...
		public async Task<APIOperationResponse<bool>> SubmitSupplyAsync(long id, SubmitSupplyDto inputDto, List<IFormFile> files)
		{
			try
			{
				...
				try
				{
					...
					// Approve the order after the supply is submitted
					if (orderId > 0)
					{
				...
				{
					await _transactionManager.RollbackAsync();
					_logger.LogError(ex, "Error in transaction while submitting supply. SupplyId: {SupplyId}, UserId: {UserId}", id, _currentUserService.UserId);
					return APIOperationResponse<bool>.Fail(ResponseType.InternalServerError, $"An error occurred: {ex.Message}");
				}
			}
			catch (Exception ex)
