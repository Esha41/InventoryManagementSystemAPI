...
{
    /// <summary>
    /// Inventory dashboard monitoring: weapon assets (depot-scoped like <see cref="Inventories.Services.InventoryService"/>)
    ...
    public class InventoryDashboardMonitoringService : IInventoryDashboardMonitoringService
    {
        ...
        private IQueryable<Data.Entities.AssetSupply> ApplyPipelineDepotFilterToAssetSupplyQuery(IQueryable<Data.Entities.AssetSupply> q, DepotScope scope)
        {
            if (!scope.RequiresPipelineDepotFilter)
                return q;
            if (!scope.PipelineDepotIds.Any())
        ...
        private static bool IsOrderAwaitingFulfillment(
            ...
            List<Data.Entities.AssetSupply> assetSupplies)
        {
